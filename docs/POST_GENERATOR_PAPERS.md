# Papers Post Generator 스펙

> arXiv 링크 → Papers 포스트 파일 세트를 생성하는 파이프라인.
> 논문 요약 품질/스타일 규칙은 `docs/PAPERS_SUMMARY_RULES.md` 참조.
...
- **`docs/PAPERS_SUMMARY_RULES.md`에 따라** 수행
- 입력: PDF 전문
- 출력: TL;DR, 문제, 핵심 아이디어, 구체적 방법, 주요 결과, 달성점과 한계점, 참조논문, card_summary
- **주요 수식 LaTeX 추출**: 핵심 방정식을 LaTeX로 변환하여 MDX 본문에 `$...$` / `$$...$$`로 삽입 (수식 작성 규칙은 `PAPERS_SUMMARY_RULES.md` 참조)
...
- `post_number`: `posts/index.json`의 마지막 번호 +1 (없으면 `node /Users/terrytaewoongum/Codes/personal/terryum-ai/scripts/generate-index.mjs` 먼저 실행)
...
### Step 6) 인덱스 재생성
- `node /Users/terrytaewoongum/Codes/personal/terryum-ai/scripts/generate-index.mjs` 실행 → `posts/index.json` 갱신

### Step 7) 검증 + Git push
- SKILL.md Step R10~R12 공통 검증 체크리스트 수행
...
| `title` | 제목 |
| `summary` | 2-3문장 요약 |
| `card_summary` | 카드용 짧은 요약 (작성 규칙은 PAPERS_SUMMARY_RULES.md) |
...
- [ ] `node /Users/terrytaewoongum/Codes/personal/terryum-ai/scripts/copy-post-images.mjs` 실행 → public 디렉토리에 복사 확인
...
### 빌드 검증
- [ ] `cd /Users/terrytaewoongum/Codes/personal/terryum-ai && rm -rf .next && npm run build` 성공
- [ ] ko/en 모두 SSG 렌더링 확인
